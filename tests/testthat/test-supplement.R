...
test_that("validator strips ANSI escape codes from a spec-parse-failure WARN message", {
  supp <- .write_supp(.supp_minimal(list(`14.1.1` = list(
    title = "x", analysis_type = "CONTINUOUS", is_supported = TRUE,
    analyses = list(list(rowLabel = "Age", variable = .av("ADSL", "AGE")))))))
  out <- withr::with_options(list(cli.num_colors = 256), {
    suppressMessages(ars_validate_supplement(
      supp, adam_spec_path = tempfile(fileext = ".xlsx")))  ## does not exist
  })
  msg <- out$problem[out$severity == "WARN" & out$where == "spec"]
  expect_false(grepl("\033", msg, fixed = TRUE))
