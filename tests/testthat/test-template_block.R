...
test_that("a categorical mock block survives as an expansion template", {
  ...
  ## The block's placeholder cells are pending FOR THE RIGHT REASON: the
  ## rows are a recorded template awaiting expansion, not orphans no
  ## analysis covers.
  cells <- re$outputs[[1]][["_meta"]][["shell_fill"]]$cells
  tpl_cells <- Filter(function(c) c$row %in% 6:8, cells)
