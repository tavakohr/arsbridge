...
The package ships the CDSC-ALZ-201 training study – a synthetic
Alzheimer’s study on the public pharmaverse (CDISC pilot Xanomeline)
data – so you can run the entire pipeline before you own a study. This
is the best way to get a feel for the output and the validation report.
...
# Or read the study's Excel shell instead of the Word one:
res <- spec_to_ars_example(shell_format = "xlsx")

# ARS JSON and validation report land in tempdir(); paths are in res.
res$n_tlfs        # 8 TLF shells (5 tables, 2 listings, 1 figure)
res$n_analyses    # ~35 analyses
...
it makes about 8 LLM calls (one per TLF section).
...
arsbridge_example("annotated_shell.xlsx")
...
| File | Purpose |
|----|----|
| `annotated_shell.docx` | TLF shells with red ADaM annotations, Word (the input) |
| `annotated_shell.xlsx` | the Excel counterpart – the shell [`ars_fill_shell()`](https://tavakohr.github.io/arsbridge/reference/ars_fill_shell.md) writes back filled |
| `adam_spec.xlsx` | ADaM variable spec with codelists (the grounding truth) |
| `ADaM.zip` | five pharmaverseadam datasets as XPT (for [`ars_to_ard()`](https://tavakohr.github.io/arsbridge/reference/ars_to_ard.md)) |
