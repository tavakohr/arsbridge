...
> **No API key, or it's blocked?** You can still run at near-LLM accuracy with
> no API call. Start with `ars_copilot_instructions()`, which sets up a
> workflow where a chat assistant (Copilot/ChatGPT) produces a supplement file
> `spec_to_ars()` consumes. See `vignette("no-api-access")`.
