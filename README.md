...
| **The filled shell, back in Excel** | For an Excel shell, `ars_fill_shell()` returns the author's own workbook with the results in it and the red annotations gone. The layout, labels, merges, fonts and footnotes are never rebuilt — only left alone — and each placeholder's own `xx.x` decides how its number is formatted. A cell whose result does not exist keeps its placeholder and is reported, because a blank cell in a clinical table reads as a zero. |
...
You do not need any external files to run the full pipeline. The package ships with a complete clinical study example (40 TLF shells, simulated ADaM data) so you can see the whole thing before touching your own study.
...
Listings and figures are not filled yet; their sheets are stripped of annotations and otherwise left as they are.
