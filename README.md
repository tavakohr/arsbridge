...
- **An unfillable cell keeps its placeholder** and appears in `res$diagnostics` with the reason — no result in the ARD, or reserved for manual derivation. An empty cell in a clinical table reads as a zero. Pass `keep_pending_placeholders = FALSE` to blank them instead, for a workbook going to someone who will complete it by hand. An arm with no qualifying subject is *not* one of these cases: it is filled with `0 (0.0)`, because that is the result — see the ARD's zero-group completion above.
