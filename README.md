...
6.  **Partial Results with Intact Traceability**: A table that mixes
    ...
    validated script and `ars_validate_manual_fills()` blocks any
    untraceable value before render. Every value – computed
    (`{cards}`/`{cardx}`/base R) or manual – carries provenance columns,
    so the whole table is auditable.
...
(via `{cardx}`) and a stratified **Cochran-Mantel-Haenszel** p-value
(base R). arsbridge detects these in the shell, classifies them to the
right method, and builds them automatically when their operands are
present (e.g. a CMH stratification variable); it falls back to a
reserved cell when a prerequisite is missing (`{cardx}` not installed,
no strata named).
...
back into the ARD (`ars_manual_worklist()` lists what is outstanding;
`ars_validate_manual_fills()` blocks an untraceable value before
...
interval), also install the optional `{cardx}`. Without it, those cells
degrade gracefully to reserved `manual_pending` placeholders rather than
erroring.
...
`MTH_PROPORTION_CI_EXACT` $\rightarrow$
`arsbridge::ard_proportion_ci_exact()` (wraps
`cardx::ard_categorical_ci()`; exact Clopper-Pearson interval). -
`MTH_CMH_TEST` $\rightarrow$ `arsbridge::ard_cmh_test()` (stratified
Cochran-Mantel-Haenszel p-value; base R, needs a `strata` operand).
...
`ars_render_all()` runs this check automatically and raises a blocker
for any untraceable manual value, so it can never reach the final
document.
