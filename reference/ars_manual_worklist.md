...
[`ars_to_ard()`](ars_to_ard.md) – statistics arsbridge could not compute
(a declared-but-unexecutable method, e.g. a Cochran-Mantel-Haenszel
p-value) and reserved as keyed stub rows. This is the analyst's
checklist: each row must be computed with a validated analysis script
and written back into the ARD (set `stat`,
`result_status = "manual_filled"`, `value_source`, `derivation_ref`)
before the table is final. See
[`vignette("getting-started")`](../articles/getting-started.md) and the
ADRs under `docs/adr/` for the round-trip.
