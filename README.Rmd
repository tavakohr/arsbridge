...
into a CDISC Analysis Results Standard (ARS) v1.0 ARM-TS JSON file, ready
for consumption by [`siera::readARS()`](https://pharmaverse.github.io/siera/).
...
- **Style-agnostic annotation detection.** Four detection layers:
  red `C00000` font → bold/italic/underline → plain-text ADaM regex →
  LLM fallback. Works on shells that follow your team's house style,
  whatever it is.
...
before define.xml exists). The SDTM spec is **not** a valid input --
TLF annotations reference ADaM variables, so the grounding source has
to be the ADaM spec.
...
study (`APX-DRM-301`): 40 TLFs (24 tables + 10 listings + 6 figures),
8 ADaM domains, 60 subjects stratified by treatment arm. The synthetic
ADaM data is not consumed by `arsbridge` itself -- it is for testing
the downstream consumer (`siera`) end-to-end.
