...
You do not need any external files – or an API key – to run the full
pipeline. The package ships the **CDSC-ALZ-201 training study**: a
synthetic Alzheimer’s study whose shells are annotated in both Word and
Excel and whose datasets are the public CDISC pilot Xanomeline data from
...
And because the bundle also carries an **Excel** shell, the
filled-workbook deliverable works offline too:
