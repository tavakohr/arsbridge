...
automates both halves. It reads the annotations directly from the Word
document, checks every variable against your ADaM spec, generates a
CDISC Analysis Results Standard (ARS) JSON, executes it against real
ADaM datasets with
...
is the main entry point. Point it at your annotated Word shell and ADaM
spec; it writes a CDISC ARS JSON and an Excel validation report.
...
recorded as a blocking finding that names the row and the rejected
token. The spec is the ground-truth oracle.
