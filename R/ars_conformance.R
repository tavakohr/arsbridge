...
.strip_ars_extensions <- function(ars) {
  ...
  ars[["outputs"]] <- lapply(ars[["outputs"]] %||% list(), function(output) {
    ...
    output[["displays"]] <- lapply(
      ...
      function(entry) {
        ## Extensions live inside the official OrderedDisplay wrapper. A
        ## displays entry in any other shape is left alone, so everything
        ## wrong with it is reported rather than partially tidied.
...
#' [spec_to_ars()]), integer `version`s, displays in the official
#' `OrderedDisplay` wrapper, terminology-object `fileType`s, valid
#' operation-role terms, and named contents-list entries -- so a fresh file
#' validates clean once the extensions are stripped.
#'
...
#' carries no compatibility readers for its own older shapes: the remedy for
#' an outdated file is regenerating it with [spec_to_ars()].
#'
