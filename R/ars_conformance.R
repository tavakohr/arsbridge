...
.strip_ars_extensions <- function(ars) {
  ...
  ars[["outputs"]] <- lapply(ars[["outputs"]] %||% list(), function(output) {
    ...
    output[["displays"]] <- lapply(
      ...
      function(entry) {
        ## Current files wrap the display (OrderedDisplay); older ones are
        ## flat. Strip inside whichever object actually carries the fields.
        if (is.list(entry[["display"]])) {
          result <- .drop_fields(entry[["display"]],
                                 .ARS_EXTENSION_FIELDS$output_display,
                                 "display", stripped)
          entry[["display"]] <- result$node
        } else {
          result <- .drop_fields(entry, .ARS_EXTENSION_FIELDS$output_display,
                                 "display", stripped)
          entry <- result$node
        }
        stripped <<- result$stripped
...
#'
#' @section What a freshly generated event reports:
#' Nothing. As of arsbridge 0.1.0.9012 the generator emits everything the
#' standard requires -- `reason`/`purpose` terminology on every analysis
#' (defaults settable in [spec_to_ars()]), integer `version`s, displays in
#' the official `OrderedDisplay` wrapper, terminology-object `fileType`s,
#' valid operation-role terms, and named contents-list entries -- so a fresh
#' file validates clean once the extensions are stripped.
#'
#' Files generated by arsbridge before 0.1.0.9012 predate those changes and
#' will report exactly those divergences. Regenerate them, or correct them in
#' [edit_ars()].
#'
...
.tidy_schema_errors <- function(errors) {
  ...
  ## ajv names the offending property in params rather than in the message;
  ## the column only exists when at least one additional-property error
  ## remains.
  params <- errors[["params"]]
  extras <- if (is.data.frame(params)) params[["additionalProperty"]] else NULL
  if (!is.null(extras)) {
    for (i in seq_along(problem)) {
      extra_property <- unlist(extras[i])
